...
|15|SIGTERM|TERM|실행중인 프로그램을 정상적인 종료방법으로 프로그램을 종료하는 신호(Signal)로 kill 명령에서 신호(Signal)를 지정하지 않으면 이 신호(Signal)를 사용하여 프로그램을 종료한다.|
|18|SIGCONT|CONT|중지 되어 있는 프로그램을 재실행 하는데 사용되는 신호(Signal)이다|
|19|SIGSTOP|STOP|프로그램을 중지 하는데 사용되는 신호(Signal)이다|
|20|SIGTSTP|TSTP|터미널에서 중지되어 있는 신호(Signal)이다|

- 신호(Signal)은 번호, 신호(Signal)이름으로 신호(Signal)로 보낼 수 있습니다
- 또한, 위 표에 옵션은 자주 사용되는 신호(Signal)로 강제로 종료시킬때 사용되는 -9 -SIGKILL 등등 옵션을 주어 사용하면 된다
